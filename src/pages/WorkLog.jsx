...
function buildEntries(date) {
  ...
  // Today: fixed realistic pattern (6 reported, 4 not)
...
export default function WorkLog() {
  ...
  // Rebuild entries when date changes
  useEffect(() => {
  ...
  return (
    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 20, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map(entry => (
            <ContractorCard
              key={entry.id}
              entry={entry}
              reminderSent={remindersSent.has(entry.id)}
      ...
      {viewEntry && (
        <ViewReportModal entry={viewEntry} onClose={() => setViewEntry(null)} />
      )}
    </div>
  );
}
...
function ContractorCard({ entry, reminderSent, onReminder, onView }) {
  const [hov, setHov] = useState(false);
  const reported = entry.status === 'reported';

  return (
    <div
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ background: 'white', borderRadius: 14, border: `1.5px solid ${hov ? (reported ? '#86efac' : '#fca5a5') : '#e2e8f0'}`, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14, transition: 'all 0.15s', boxShadow: hov ? '0 4px 16px rgba(0,0,0,0.07)' : '0 1px 3px rgba(0,0,0,0.03)', flexWrap: 'wrap' }}
    >
      {/* Status indicator strip */}
      <div style={{ width: 4, height: 44, borderRadius: 4, background: reported ? '#22c55e' : '#ef4444', flexShrink: 0 }} />
      ...
      <div style={{ flex: '0 0 auto', textAlign: 'center', minWidth: 110 }}>
        {reported ? (
          ...
          <>
            ...
            {reminderSent && (
              <p style={{ margin: '4px 0 0', fontSize: 11, color: '#16a34a' }}>🔔 תזכורת נשלחה</p>
            )}
          </>
        )}
      </div>
      ...
      <div style={{ flex: '0 0 auto' }}>
        {reported ? (
          <ActionBtn onClick={onView} color={TEAL} textColor="white">
            צפה בדוח
          </ActionBtn>
        ) : (
          <ActionBtn
            onClick={reminderSent ? undefined : onReminder}
            color={reminderSent ? '#f0fdf4' : '#1e293b'}
            textColor={reminderSent ? '#16a34a' : 'white'}
            border={reminderSent ? '#86efac' : undefined}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div>
            ...
            <input type="number" min={1} max={99} value={form.workers} onChange={setN('workers')}
              style={{ width: '100%', padding: '10px 14px', border: '1.5px solid #e2e8f0', borderRadius: 10, fontSize: 14, color: '#1e293b', outline: 'none', background: '#f8fafc', boxSizing: 'border-box', textAlign: 'center', fontWeight: 700, fontSize: 18 }} />
          </div>
        </div>
...
function ViewReportModal({ entry, onClose }) {
  const style = TRADE_STYLE[entry.trade] ?? { bg: '#f1f5f9', text: '#475569', border: '#e2e8f0' };
  return (
...
function Overlay({ children, onClose, maxWidth = 520 }) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, backdropFilter: 'blur(2px)' }}
